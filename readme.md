...
Каждый интерфейс это ненаправленное ребро, раскладывающееся на два линка смежности, что упоминалось выше. Адресация интерфейсов (ip-сегментов) не приводится, чтобы не загромождать рисунок. Указаны только loopback-сети и локальные стоимости всех интерфейсов. Loopback это тоже интерфейс, требующий только одного линка смежности для описания. На рисунке так же указаны два "тупиковых" сегмента, смотрящие в сторону коммутаторов. Понятие "тупиковый" в разных контекстах может означать немного разные вещи. В нашем случае подразумевается, что в таких сегментах нет смежности с другими узлами, поэтому тупиковое ребро тоже описывается лишь одним линком смежности.

Стоимость (cost) может быть нескольких типов :
...
Локальная стоимость ip-сегмента = локальной стоимости интерфейса, которому назначен этот сегмент.
...
кумулятивная стоимость ip-сегмента = кумулятивная стоимость до узла + локальная стоимость интерфейса, которому назначен ip-адрес из данного сегмента.
...
Первые две строки описывают ненаправленное ребро, т.е. сегмент сети между узлами S и A (либо A и S, что одно и то же). Поскольку это полноценные маршрутизаторы, то здесь **два** линка смежности для одного сегмента.

Третья строка описывает тупиковый сегмент, т.к. мы не знаем про существование каких-либо узлов за коммутатором. Именно поэтому поля neighIP и neighID равны 0.0.0.0, что тоже важный момент, на который стоит обратить внимание. С помощью таких значений мы указываем реализации алгоритма, что не нужно искать узлы за этим интерфейсом.

Последние две строки описывают лупбеки. Как и тупиковые сети, они не участвуют в рассчётах. Лупбек это локальный петлевой интерфейс, смотрящий обратно на узел. Маска необязательно должна быть /32, но вот поля localIP, neighIP, neighID обязательно должны быть равны NodeID.
